...
NumericVector squat_single_binom_bidir_g(int x, int n, double p, double alpha, double beta, 
  ...
  if ( var_adj ) {  // when variance adjustment is enabled, calculate new variance
    int i = 0;
    double sum = 0.0;
    double ez, f0, f1, gf0, gf1, den;
    f0 = 0.0;
    if ( approx_under == 0 ) { // calculate exact variance in log-scale to avoid underflow (could be slow)
      for(i=0; i <= n; ++i) {
        den = R::dbinom(i, n, p, false); // f(x)=Pr(x=i)
        f1 = f0 + den; 
        ez = expt_truncated_bidir_gamma_from_qt(f0,f1,alpha,beta,0,1);
        f0 = f1;
        if (!std::isnan(ez)) sum += den * pow( (ez-mu), 2 ); // otherwise, add E[z|x]^2f(x)        
      }
    }
      ...
      sum = 0;
      int np = (int)(n*p); // median is somewhere between np and np+1
      fnp = R::pbinom(np, n, p, true, false);
      gfnp= R::pgamma(R::qgamma(1-2*f0,alpha,1/beta,true,false),alpha+1,1/beta,1,0);
      den = R::dbinom(np+1, n, p, false);     
      f0 = R::pbinom(np, n, p, true, false);
      f1 = R::pbinom(np+1, n, p, true, false);
      ez = expt_truncated_bidir_gamma_from_qt(f0,f1,alpha,beta,0,1);
      sum += den * pow(ez-mu, 2);
      i = np+2;
      if (np+1 < n) {
        f0 = f1;
        gf0= R::pgamma(R::qgamma(2*f0-1,alpha,1/beta,true,false),alpha+1,1/beta,1,0);           
        for(i=np+2; i <= n; ++i) { // add upper-tail exact propobabilities
          den = R::dbinom(i, n, p, false); // f(x)=Pr(x=i)
          f0 = f0 + den; 
          gf1 = R::pgamma(R::qgamma(2*f0-1,alpha,1/beta,true,false),alpha+1,1/beta,1,0);
          ez = alpha / beta * (gf1 - gf0) / den / 2;
          gf0 = gf1;
          if ( ( den < approx_under ) && ( i - 1 > n * p ) ) 
            break; // stop exact calculation and perform approximation
          sum += den * pow( (ez-mu), 2 ); // otherwise, add E[z|x]^2f(x)
      ...
      for(i=np; i >= 0; --i) { // add lower-tail exact probabilities
        ...
        gf0= R::pgamma(R::qgamma(1-2*f0,alpha,1/beta,true,false),alpha+1,1/beta,1,0);
        ez = alpha / beta * (gf0 - gf1) / den / 2;
        gf1 = gf0;
        if ( den < approx_under )
          break; // stop exact calculation and perform approximation
        sum += den * pow( (ez-mu), 2 ); // otherwise, add E[z|x]^2f(x)
...
// [[Rcpp::export]]
List squat_multi_binom_unidir_g(IntegerVector xs, NumericVector sizes, NumericVector ps, bool var_adj = true, double approx_under = 1e-4) {
  ...
  for(int i=0; i < n; ++i) {
    res = squat_single_binom_unidir_g(xs[i], 
                                      (sizes.size() == 1) ? sizes[0] : sizes[i], 
                                                                            (ps.size() == 1) ? ps[0] : ps[i], 
                                                                                                         alpha[i], beta, var_adj, approx_under);
    zs[i] = res[0];
    v_z += res[1]*res[1];
  }
