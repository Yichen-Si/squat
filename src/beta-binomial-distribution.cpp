...
double cpp_pbbinom_one(
  ...
  double mx = std::min(x, size);
  
#ifdef IEEE_754
  if (ISNAN(x) || ISNAN(size) || ISNAN(alpha) || ISNAN(beta)) {
    p = x + size + alpha + beta;
  }
#endif
  
  if (alpha <= 0.0 || beta <= 0.0 ||
      size < 0.0 || !isInteger(size, false)) { 
    throw_warning = true;
    p = NAN;
  } else if (x < 0.0) {
    p = 0.0;
    ...
    p = 1.0;
  } else if (is_large_int(x)) {
    p = NA_REAL;
    warning("NAs introduced by coercion to integer range");
  } else {
    std::vector<double> tmp = cdf_bbinom_table(mx, size, alpha, beta);
    p = tmp[to_pos_int(x)];    
  }
  
  if (p < SMALL) p = SMALL;
  else if (p > 1 - SMALL) p = 1. - SMALL;  
  if (!lower_tail)
    p = 1.0 - p;  
  if (log_prob)
    p = log(p);
...
double cpp_dbbinom_one(
  ...
  double p; 
  bool throw_warning = false;
  p = logpmf_bbinom(x, size, alpha, beta, throw_warning);
  if (!log_prob)
    p = exp(p);
  if (throw_warning)
